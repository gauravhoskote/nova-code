...
import { NovaProcess, NovaMessage } from './novaProcess';

export class ChatPanel {
  static readonly viewType = 'novacode.chat';
  private static _instance: ChatPanel | undefined;

  private readonly _panel: vscode.WebviewPanel;
  private _nova: NovaProcess;
  private readonly _extensionUri: vscode.Uri;
  private readonly _disposables: vscode.Disposable[] = [];

  // Named listener so we can detach it when swapping the process
  private _novaListener: ((msg: NovaMessage) => void) | undefined;
  private _novaExitListener: (() => void) | undefined;
  ...
  private _webviewLoaded = false;
  private _pendingMessages: NovaMessage[] = [];

  // Tracks the last focused text editor so context is available even after
  // the user clicks into the chat webview (which clears activeTextEditor).
  ...
  private _wireNova(nova: NovaProcess): void {
    ...
    this._novaListener = (msg: NovaMessage) => {
      if (msg.type === 'sessions_list') {
        this._handleSessionsList(msg as unknown as { sessions: Array<{ path: string; title: string; created_at: string }> });
        return;
      }
  ...
  private async _handleSessionsList(
    msg: { sessions: Array<{ path: string; title: string; created_at: string }> },
  ): Promise<void> {
    type SessionItem = vscode.QuickPickItem & { path: string | null };
  ...
  private _dispose(): void {
    if (this._novaListener)     { this._nova.off('message', this._novaListener); }
    if (this._novaExitListener) { this._nova.off('exit', this._novaExitListener); }
  ...
  private _buildHtml(): string {
    ...
    try {
      markedJs = fs.readFileSync(
        path.join(this._extensionUri.fsPath, 'node_modules', 'marked', 'marked.min.js'),
        'utf8',
      );
    } catch {
